  ...
  /*
   * Android: binder ioctl , some of them need custom do_ioctl
   * Original is BINDER_WRITE_READ, BINDER_SET_MAX_THREADS, BINDER_VERSION
   */
  IOCTL_SPECIAL(BINDER_WRITE_READ, IOC_RW, do_ioctl_binder_write_read, MK_PTR(MK_STRUCT(STRUCT_binder_write_read))) /* has ptr */
  IOCTL(BINDER_SET_IDLE_TIMEOUT, IOC_W, MK_PTR(TYPE_LONG)) /* __s64 */
  IOCTL(BINDER_SET_MAX_THREADS, IOC_W, MK_PTR(TYPE_INT))
  IOCTL(BINDER_SET_IDLE_PRIORITY, IOC_W, MK_PTR(TYPE_INT))
  IOCTL(BINDER_SET_CONTEXT_MGR, IOC_W, MK_PTR(TYPE_INT))
  IOCTL(BINDER_THREAD_EXIT, IOC_W, MK_PTR(TYPE_INT))
  IOCTL(BINDER_VERSION, IOC_RW, MK_PTR(MK_STRUCT(STRUCT_binder_version)))
  IOCTL(BINDER_GET_NODE_DEBUG_INFO, IOC_RW, MK_PTR(MK_STRUCT(STRUCT_binder_node_debug_info))) /* has ptr */
  IOCTL(BINDER_GET_NODE_INFO_FOR_REF, IOC_RW, MK_PTR(MK_STRUCT(STRUCT_binder_node_info_for_ref)))
  IOCTL(BINDER_SET_CONTEXT_MGR_EXT, IOC_W, MK_PTR(MK_STRUCT(STRUCT_flat_binder_object))) /* has ptr */
  ...
  IOCTL(BINDER_ENABLE_ONEWAY_SPAM_DETECTION, IOC_W, MK_PTR(TYPE_INT))
  IOCTL(BINDER_GET_EXTENDED_ERROR, IOC_RW, MK_PTR(MK_STRUCT(STRUCT_binder_extended_error)))

  /*
   * Android ashmem ioctl
   */
  IOCTL(ASHMEM_SET_NAME, IOC_R, MK_PTR(TYPE_CHAR)) /* char[256]*/
  IOCTL(ASHMEM_GET_NAME, IOC_W, MK_PTR(TYPE_CHAR)) /* char[256]*/
  IOCTL(ASHMEM_SET_SIZE, IOC_R, TYPE_ULONG)
  ...
  IOCTL(ASHMEM_GET_FILE_ID, IOC_W, TYPE_ULONG)
